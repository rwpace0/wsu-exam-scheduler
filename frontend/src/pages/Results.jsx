  ...
  useEffect(() => {
    const fetchExams = async () => {
      const response = await fetch(
        `http://127.0.0.1:5000/search?q=${encodeURIComponent(searchVal)}`,
      );
      ...
      {exams.length > 0 ? (
        <ul>
          {exams.map((exam, index) => {
            ...
            return (
              <li key={index}>
                {exam.primary}, {exam.campus}, {exam.section}, {exam.day},{" "}
                {exam.time}
                <button
                  onMouseEnter={() => setHover(exam.section)}
                  onMouseLeave={() => setHover(null)}
                  onClick={() => handleToggleClass(exam)}
                  className="bg-customCrimson ml-2 rounded-lg p-2 font-semibold text-white transition-colors hover:bg-red-800"
                >
                  {!isAdded
                    ? "Add"
                    : hover === exam.section
